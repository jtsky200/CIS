...
// Aggressive PDF splitting system for large owner manuals
async function splitPDFAggressive(filePath, maxSizeMB = 0.5) {
    ...
    try {
        ...
        // Calculate how many chunks we need (aggressive splitting)
        const maxChunkSizeBytes = maxSizeMB * 1024 * 1024;
        const totalChunks = Math.ceil(fileBuffer.length / maxChunkSizeBytes);
        
        console.log(`   📊 Aggressive splitting into ${totalChunks} chunks of max ${maxSizeMB}MB each`);
        ...
        // Split the file into very small chunks
...
// Upload split PDF chunks with retry logic
async function uploadSplitChunksWithRetry(chunks, originalFile, vehicleType, category) {
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        while (retryCount < maxRetries && !success) {
            try {
                ...
                console.log(`   📤 Attempt ${retryCount + 1}/${maxRetries}: Uploading ${Math.round(base64Content.length / 1024 / 1024)}MB...`);
...
async function main() {
    ...
    console.log('🚀 Aggressive PDF Splitting and Upload System');
    ...
    for (let i = 0; i < largePDFs.length; i++) {
        ...
        // Aggressive split the PDF file (0.5MB chunks)
        const splitResult = await splitPDFAggressive(filePath, 0.5);
        ...
        if (splitResult.success) {
            if (splitResult.chunks.length === 1 && splitResult.message === 'File already small enough') {
                ...
                console.log(`   📊 Aggressive split into ${splitResult.chunks.length} chunks, uploading...`);
                
                // Upload all chunks with retry logic
                const uploadResult = await uploadSplitChunksWithRetry(splitResult.chunks, path.basename(filePath), vehicleType, category);
    ...
    console.log('\n📊 AGGRESSIVE PDF SPLITTING AND UPLOAD SUMMARY:');
...
// Run the aggressive PDF splitting and upload system
main().then(result => {
    console.log('\n🎉 Aggressive PDF Splitting and Upload System Completed!');
    ...
    if (result.totalSuccessCount > 0) {
        ...
        console.log('🔍 The aggressive splitting system handles large files efficiently');
    ...
    console.error('❌ Aggressive PDF splitting and upload process failed:', error);
});
